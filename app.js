...
app.get('/admin', (req, res, next) => {
    ...
    if (userData) {
        ...
        if (userRole === 'admin') {
            ...
            database.query('SELECT * FROM account JOIN person USING(Id_P)', (err, results) => {
            if(err) throw err;
            console.log('Database query results:', results);
            res.render('admin', { users: JSON.parse(userData), l: results });
            });
        } else {
            console.log(`Access denied for user ${user.username} with role ${userRole}.`);
            res.render('stars', { users: JSON.parse(userData), errorAdmin: 'Access denied: Admins only'});
        }
    } else {
...
app.delete('/delete/:Id_P', (req, res) => {
    ...
    database.query(sqlDeleteAccount, [Id_P], function(err, result) {
        if (err) {
            ...
            return res.status(500).json({ error: "An error occurred while deleting from account" });
        }
        database.query(sqlDeletePerson, [Id_P], function(err, result) {
            if (err) {
                console.error('An error occurred while deleting from person:', err);
                return res.status(500).json({ error: "An error occurred while deleting from person" });
            }
            console.log(`Deleted Account and associated personal info with Id ${Id_P}`);
...
app.get('/solarsyst', (req, res, next) => {
    // get the token from the cookie
    const token = req.cookies.jwt;
    console.log("\nToken verification...")
            ...
            if (error) {
                ...
                database.query(sqlPlanetDisplay, [Id_Css], (error, sqlResults) => {
                    if (error) {
                        ...
                        res.render('custom', { solarSystem: sqlResults.length > 0 ? sqlResults : [] });
                    }
                });
            }
        });
...
app.post('/toregister', (req, res) =>{
    const { name, lastname, birth_date, username, email, password, confirmPassword, role_id } = req.body;
    ...
    const sqlPerson = 'INSERT INTO person (name, lastname, birth_date) VALUES (?, ?, ?)';
    ...
    const sqlAccount = 'INSERT INTO account (username, email, password, role_id,Id_P) VALUES(?, ?, ?, ?, LAST_INSERT_ID())';
        ...
        // Vérification email
            database.query(uniqueEmail, [email], (error, results) => {
                if(error){
                    ...
                    return res.render('register', { error: 'This email is already taken !'});
        } 
        else{
            // Password hashing
            bcrypt.genSalt(saltRounds).then(salt =>{
                ...
                console.log(`New user: ${username}, ${name}, ${birth_date}, ${email}`);
                //Génération du token
                ...
                */
                res.cookie('jwt', token, { httpOnly: true, maxAge: ThreeHours * 1000 })
        ...
        .then(passwordMatch => {
            if (passwordMatch){
                ...
                if (decodedUsername === user.username){
                    console.log(`${decodedUsername} is connected with the email ${user.email}`);
                    res.cookie('jwt', token, { httpOnly: true, maxAge: ThreeHours * 1000 });
                    console.log('Token cookie created ! \nCreating user data...');
                    res.cookie('userData', JSON.stringify(sqlResults), { httpOnly: true, maxAge: ThreeHours * 1000 });
...
app.post('/become-admin', (req, res) => {
    ...
    if (secretCode === correctSecretCode) {
        ...
        database.query(updateRoleQuery, [adminRoleId, username], (error, results) => {
            if (error) {
            ...
            database.query(userDataQuery, [username], (error, userDataResults) => {
                ...
                res.cookie('userData', JSON.stringify(userDataResults), { httpOnly: false, maxAge: ThreeHours * 1000 });
        ...
        res.render('stars', { error: 'Incorrect secret code', users: JSON.parse(req.cookies.userData) });
    }
});
...
app.post('/createCustomSolarSystem', (req, res) => {
    ...
    const sqlCss = 'INSERT INTO Custom_Solar_System (name_Solar_System, description, Id_Acc) VALUES (?, ?, (SELECT Id_Acc FROM Account WHERE username = ?))';

    database.query(sqlCss, [name_Solar_System, description, username], (error, CustomSolarSystem) => {
        if (error) {
...
app.post('/addPlanet', (req, res) => {
    ...
    const insertPlanetQuery = 'INSERT INTO Astral_Object (astral_name, nature_object, diameter, distance, Id_Css) VALUES (?, ?, ?, ?, (SELECT Id_Css FROM Custom_Solar_System WHERE Id_Acc = (SELECT Id_Acc FROM Account WHERE username = ?)))';

    database.query(insertPlanetQuery, [astral_name, nature_object, diameter, distance, username], (error, results) => {
        if (error) {
            console.error('Database error:', error);
            return res.status(500).json({ error: 'Database error' });
        }
        ...
        database.query(updateCustomSolarSystemQuery, [username, newPlanetId, username], (error, updateResults) => {
            if (error) {
                console.error('Database error:', error);
                return res.status(500).json({ error: 'Database error' });
            }
...
app.post('/editAstral/:Id_AO', (req, res) =>{
    ...
    WHERE Id_AO = ?`;
    database.query(sqlEdit, [astral_name, nature_object, diameter, distance, listId], (error, result) => {
        if(error){
            console.error('Error updating astral object ', error);
            res.status(500).send("An error occurred while updating the astral object");
        } else {
...
app.delete('/deleteAstral/:Id_AO', (req, res) => {
    ...
    database.query(sqlDelete, [Id_AO], (error, result) => {
        if (error) {
            ...
            res.status(500).send({error: "An error occurred while deleting the astral object"});
        } else {
